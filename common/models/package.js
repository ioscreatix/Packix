...
module.exports = function (Package) {
  ...
  Package.reload = () => {
    ...
    Package.find({
      ...
      PackagesFile.end();
      //let packageDataBuffer = new Buffer(fs.readFileSync(path.resolve(__dirname + '../../../client/static/Packages')), 'utf8');
     // bufferToStreamSync(packageDataBuffer).pipe(zlib.createGzip()).pipe(fs.createWriteStream(path.resolve(__dirname + '../../../client/static/Packages.gz')));
     ...
     // fs.writeFileSync(path.resolve(__dirname + '../../../client/static/Packages.bz2'), compressed);
     // let compressedLZMA = new Buffer(lzmajs.compressFile(packageDataBuffer));
     // fs.writeFileSync(path.resolve(__dirname + '../../../client/static/Packages.lzma'), compressedLZMA);
      let packageFile = fs.createReadStream(path.resolve(__dirname + '../../../client/static/Packages'));
      ...
      var input = fs.createReadStream(path.resolve(__dirname + '../../../client/static/Packages'));
      var output = fs.createWriteStream(path.resolve(__dirname + '../../../client/static/Packages.xz'));

      input.pipe(compressor).pipe(output);

      var oCompressor = lzma.createStream('aloneEncoder', {});
      var input1 = fs.createReadStream(path.resolve(__dirname + '../../../client/static/Packages'));
      var output1 = fs.createWriteStream(path.resolve(__dirname + '../../../client/static/Packages.lzma'));
      ...
      fs.writeFileSync(path.resolve(__dirname + '../../../client/static/Release'), releaseFileString);
                        ...
                        if (packageVersionError) {
                          ...
                          Package.app.models.PackageFile.create(obj, (packageFileError, packageFileObject) => {
                            if (packageFileError) {
                              ...
                              Package.updateLatestVersion(packageObject.id).then((result) => {
                                console.log(result);
                              }).catch((err) => {
                                console.log(err);
                              });
    ...
    return new Promise(async (resolve, reject) => {
      try {
        ...
        if (price) {
          ...
          const packageAuthor = packageData['packageAuthor'];
  ...
  Package.prototype.purchasePayPal = function(ctx) {
    this.purchase(ctx, function(err, data) {
      if (data) {
        ...
        ctx.res.redirect('/package/' + this.identifier + '/');
      }
    })
  };
    ...
    try {
      ...
      if (Array.isArray(ctx.result)) {
        ...
        for (let packageObj of ctx.result) {
          if (packageObj.visible || (accountId && String(packageObj.accountId) === accountId)) {
        ...
        if (tempResult['visible'] || (accountId && String(tempResult['accountId']) === accountId)) return Promise.resolve(ctx.result);
