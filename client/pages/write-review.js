...
class WriteReview extends ReactComponent {
  constructor(props) {
    ...
    this.submitReview = async () => {
      ...
      let url = 'https://' + this.props.baseURL + '/api/PackageVersions/' + this.props.data.latestVersionId + '/review';
      ...
      let response = await fetch(
        url,
        {
          ...
          body: JSON.stringify({title: this.state.titleR, description: this.state.descriptionR})
        },
      );
      ...
      if (this.state.ratingR > 0) {
        ...
        let rateURL = 'https://' + this.props.baseURL + '/api/PackageVersions/' + this.props.data.latestVersionId + '/rate?rating=' + this.state.ratingR;
        rateURL = rateURL.replace(/http:\/\//, 'https://');
    ...
    if (req) {
     ...
     // console.log(req);
      ...
      try {
        ...
        let response = await fetch(
          'https://' + baseUrl + '/api/Packages/writeReview?id=' + packageId,
          {
            method: 'get',
            headers: {
              'x-forced-ip': ip,
      ...
      let url = "";
      url += "https://" + req.get('host') + "/package/";
      url += data["identifier"];
      url += '/review';
